...
function silhouetteClicked(event) {
  ...
  let dinosaur = clickedElement.dataset.dinosaur;
  console.log("Element that was clicked: ", dinosaur);
  displayElem.querySelector("h2").textContent = dinosaurs[dinosaur].title;
  displayElem.querySelector("img").src = `images/${dinosaurs[dinosaur].image}`;
  
  // I have NO idea why nor how this works. We just take it.
  //  https://stackoverflow.com/a/45036752
  displayElem.querySelector("img").style.animation = "asdf";
  // Things to force reflow: https://gist.github.com/paulirish/5d52fb081b3570c81e3a
  displayElem.querySelector("img").offsetHeight; // trigger DOM reflow 
  displayElem.querySelector("img").style.animation = null;

  displayElem.querySelector("p").textContent = dinosaurs[dinosaur].description;
  
  // change class of silhouettes
  const silhouettes = document.getElementsByClassName("silhouette");
  for (let silhouette of silhouettes) {
    silhouette.classList.remove("selected");
  }
